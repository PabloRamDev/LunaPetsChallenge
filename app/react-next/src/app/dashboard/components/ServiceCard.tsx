...
const ServiceCard = ({props}: {props: SlicesState}) => {
  return (
    ...
    <div className="p-5">
        ...
        <Link href={`/services/${props.id}`} className={`inline-flex items-center px-3 py-2 text-sm font-medium text-center ${props.status == 'pending' ? 'bg-red-700 rounded-lg hover:bg-red-800' : 'bg-green-700 rounded-lg hover:bg-green-800' } text-white  focus:ring-4 focus:outline-none focus:ring-blue-300`}>
        {props.status}
